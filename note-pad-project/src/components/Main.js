...
function Main() {
    ...
    return (
        <>
            {
                notes.map(note => (
                    <div key={note.id} onClick={()=>clicked(note.id)} className='w-4/5 flex flex-col md:justify-between md:flex-row bg-white h-36 justify-center gap-1 md:items-center lg:h-28 lg:w-1/2   md:h-24 md:w-11/12 mt-5 rounded-2xl'>
                        <div className='mx-4'>
